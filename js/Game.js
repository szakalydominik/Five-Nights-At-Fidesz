...
class Game {
    ...
    showTutorial(type = 'night1') {
        ...
        if (type === 'night2') {
            ...
            tutorialContent.innerHTML = `
                <h2>VÉDD MEG MAGAD A PROPAGANDÁTÓL!</h2>
                <p>
                    LÁZÁR JÁNOS MEGFOG TÁMADNI TÉGED A SZELLŐZŐKÖN KERESZTÜL,
					EZÉRT, HA DÜBÖRGÉST HALLASZ ZÁRD LE AZOKAT. MIUTÁN LEZÁRTAD
					NÉHÁNY MÁSODPERC MÚLVA ÚJRA DÜBÖRGÉST FOGSZ HALLANI, EKKOR
					KIMEGY ONNAN ONNAN ÉS FELNYITHATOD A SZELLŐZŐKET.
                </p>
                <button id="tutorial-got-it">OKÉ</button>
            `;
            // 重新绑定按钮事件
            ...
            tutorialContent.innerHTML = `
                <h2>VÉDD MEG MAGAD A PROPAGANDÁTÓL!</h2>
                <p>
                    ORBÁN VIKTOR MÁR EGYRE GYORSABB. DEUTSCH TAMÁS
					MINDIG A 6-OS KAMERÁBAN LESZ, ÉS MEGPRÓBÁL FELROBBANTANI.
					VIGYÁZZ, MERT LÁZÁR JÁNOS A SZELLŐZŐKÖN TÁMADNI FOG RÁD!
                </p>
                <button id="tutorial-got-it">OKÉ</button>
            `;
            // 重新绑定按钮事件
            ...
            tutorialContent.innerHTML = `
                <h2>VÉDD MEG MAGAD A PROPAGANDÁTÓL!</h2>
                <p>
                    ORBÁN VIKTOR MINDIG A 11-ES KAMERÁNÁL KEZD. 
					HASZNÁLD A KAMERÁK MELLETT A PLAY SOUND GOMBOT, HOGY TÁVOL TARTSD MAGADTÓL.
					GYŐZŐDJ MEG RÓLA, HOGY ABBAN A KAMERÁBAN JÁTSZOD LE A HANGOT, AMELYIK ORBÁN VIKTOR AKTUÁLIS HELYE MELLETT VAN. 
					HA UGYANAZON A HELYEN KÉTSZER VAGY TÖBBSZÖR EGYMÁS UTÁN JÁTSZOD LE A HANGOT, AZ NEM FOG MŰKÖDNI.
					A HANGOK TÚL GYAKORI LEJÁTSZÁSA A KAMERÁK MEGHIBÁSODÁSÁHOZ VEZET. 
					A JAVÍTÁSHOZ MENJ A VEZÉRLŐPULTHOZ, ÉS INDÍTSD ÚJRA A KAMERÁKAT, AHOGYAN KORÁBBAN IS TETTED.
					ORBÁN A SZELLŐZŐKÖN KERESZTÜL NEM TUD TÁMADNI. DE A KÖVETKEZŐ ÉJSZAKA MÁR AZOKRA IS KELL FIGYELNED.
    ...
    showGoldenStephen() {
        ...
        goldenImg.style.animation = 'golden-flicker 3s ease-in-out';
    ...
    toggleCamera() {
        // Mutex: ha a control panel nyitva van, ne nyíljon meg a kamera is.
        // Először zárjuk be a control panelt, csak utána kezeljük a kamerát.
    ...
    gameOver(message) {
        ...
        if (controlPanel) {
            ...
            controlPanel.style.display = 'none'; // 强制通过内联样式隐藏
            this.state.controlPanelOpen = false; // 重置逻辑状态
            this.state.controlPanelBusy = false; // 重置忙碌状态
        }
    ...
    winNight() {
		...
		// Ezzel kényszerítjük a 'display: none' alkalmazását, még akkor is, ha van !important a CSS-ben:
		cameraPanel.style.setProperty('display', 'none', 'important');
	}
    ...
    playNight5VictoryAnimation() {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                rescueText.textContent = 'ÁRAD A TISZA!';
    ...
    // Night end animation: 5:59AM -> 6:00AM -> nap a rendszerváltásig.
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                // 如果还有下一关，显示剩余天数（故事设定是5晚，所以总是显示5-当前关卡）
                    ...
                    timeDisplay.textContent = `${daysRemaining} ${daysRemaining === 1 ? 'nap' : 'nap'} a rendszerváltásig`;
                    timeDisplay.style.fontSize = '5vw';
                    timeDisplay.style.color = '#fff';
                } else {
    ...
    gameOverScreen(message, win = false) {
        this.gameOverText.textContent = 'VÉGE VAN, KICSI';
    ...
    // 1. Állapotok resetelése
    this.state.controlPanelOpen = false;
    this.state.controlPanelBusy = false;
    
    // 2. Statikus vezérlőpult kényszerített elrejtése
    const controlPanel = document.getElementById('control-panel');
    if (controlPanel) {
        controlPanel.classList.add('hidden');
        controlPanel.style.display = 'none';
        controlPanel.style.animation = 'none'; 
    }
    
    // 3. DINAMIKUS ELEMEK TAKARÍTÁSA (EZ A FONTOS!)
    // Feltételezve, hogy a 'this.ui' az UIManager példányod
    if (this.ui && typeof this.ui.cleanupUI === 'function') {
        this.ui.cleanupUI();
    }
    
    // 4. Játék motor leállítása
    ...
    showMainMenu() {
        // 1. Először állítsuk le a játék logikát (ez a biztonságos alap)
        this.stopGame();

        // 2. Vizuális elemek alaphelyzetbe állítása
        this.gameOverElement.classList.add('hidden');
        this.gameScreen.classList.remove('active');
        ...
        // KÉNYSZERÍTETT TAKARÍTÁS: A vezérlőpult teljes resetje
        const controlPanel = document.getElementById('control-panel');
        if (controlPanel) {
            controlPanel.classList.add('hidden');
            controlPanel.style.display = 'none'; // 'none'-ra állítjuk biztosan
            this.state.controlPanelOpen = false;
            this.state.controlPanelBusy = false;
        }
