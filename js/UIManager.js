...
class UIManager {
    ...
    toggleControlPanel() {
    // BIZTONSÁGI ELLENŐRZÉS: Ha a játék nem fut, ne csináljon semmit
    if (!this.game.state.isGameRunning) {
        return;
    }

    // Mutex: ha a kamera nyitva van, ne tudjon a control panel is megnyílni.
    // Először zárjuk be a kamerát, csak utána nyissuk a control panelt.
    if (this.game.state.cameraOpen && this.game.camera) {
        try { this.game.camera.close(); } catch (e) { /* ignore */ }
    }
    ...
    if (panel) {
        ...
        // Ha be akarjuk zárni a panelt, de foglalt, ne engedjük
...
// UIManager.js
cleanupUI() {
    // Eltávolítja a tooltipet
    const tooltip = document.getElementById('game-tooltip');
    if (tooltip) tooltip.remove();
    
    // Eltávolítja a control panel popupot, ha létezik
    const popup = document.getElementById('control-panel-popup');
    if (popup) popup.remove();
    
    // Kényszerített elrejtés egyéb elemeknek
    ...
    createCameraButton() {
        ...
        // Top arrow container (egy darab háromszög — user kérés)
        ...
        // Top arrow (points left when closed, rotates 180deg when open)
        ...
        arrowTop.style.transition = 'transform 0.3s';
        ...
        // Bottom arrow container (egy darab háromszög — user kérés)
        ...
        arrowBottom.style.transition = 'transform 0.3s';
    ...
    updateHotspotVisibility(viewPosition) {
        ...
        // Kamera gomb: csak akkor látszódjon, amikor a játékos TELJESEN jobbra
        // húzta a nézetet (viewPosition >= 0.97), pont úgy szimmetrikusan mint
        // a control panel (< 0.15 bal). Kivétel: ha a kamera NYITVA van, a
        // gomb látható marad, hogy a user be tudja zárni.
        if (cameraBtn) {
            const isRightView = viewPosition >= 0.97;
            const isCameraOpen = !!(this.game && this.game.state && this.game.state.cameraOpen);
            const isVisible = isRightView || isCameraOpen;
